function h = plot_2d_barco_ga( tipo,resultado,otimizacao)
h = {};
for i = 1:length(resultado.redes)
    for j = 1:length(tipo)
        ...
        hold off;
        title(sprintf('Melhor Velocidade para a Distacia %f nm é %f Km/h do Treino %d',otimizacao.distacia,resultado.redes(i).ga(tipo(j)).x,tipo(j)));
